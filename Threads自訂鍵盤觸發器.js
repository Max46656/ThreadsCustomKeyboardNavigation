...
class boardInputNavigation {
    ...
    setEventListeners() {
        ...
        const boardKey = GM_getValue("boardKey", { Home: "F", Notifications: "D", Profile: "S", Search: "A", Create: "G" });
    ...
    handleKeydown(event) {
        ...
        for (const [key, element] of Object.entries(boardKey)) {
            if (event.key.toUpperCase() === element.toUpperCase()) {
                console.log("to",key.toLowerCase());
                keyMap[key].click();
                break;
            }
        }
...
class inputCustomMenu{
    ...
    loadBoardKey() {
        this.boardKey = GM_getValue("boardKey", { Home: "F", Notifications: "D", Profile: "S", Search: "A", Create: "G" });
    ...
    registerMenuCommands(instance) {
        const buttonTypes = ["Home", "Notifications", "Profile", "Search", "Create"];
